...
def load_models():
    ...
    for fold in range(1, config.N_FOLDS + 1):
        ...
        if not path.exists():
            print(f"⚠️ Modelo do fold {fold} não encontrado, ignorando.")
            continue
...
def training_report():
    ...
    if not log_file.exists():
        print("\n⚠️ Nenhum log encontrado! Execute primeiro o treino para gerar métricas.")
        return
    ...
    if fold_scores:
        ...
        print("⚠️ Nenhuma métrica encontrada nos logs.")
...
if __name__ == "__main__":
    ...
    # Salvar máscara colorida e bruta
