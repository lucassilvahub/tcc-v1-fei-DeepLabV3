...
if __name__ == "__main__":
    ...
    params["results_folder"] = (
        f"output_35/K1x5noAug_{params['model']['name']}b45drop2_imgnet_{params['optimizer_params']['optimizer']}{params['optimizer_params']['weight_decay']}WD_{params['loss']['name']}1.0-0.5_noWeight"
    )
    ...
    folds_dir = os.path.join(params["root_dir"], "folds")

    # Load image and label files from .txt
    train_images1 = pd.read_table(f"{folds_dir}/fold1_images.txt", header=None).values
    train_images2 = pd.read_table(f"{folds_dir}/fold2_images.txt", header=None).values
    train_images3 = pd.read_table(f"{folds_dir}/fold3_images.txt", header=None).values
    train_images = [
        os.path.join(image_dir, f[0])
        for f in np.concatenate([train_images1, train_images2, train_images3])
    ]

    train_labels1 = pd.read_table(f"{folds_dir}/fold1_labels.txt", header=None).values
    train_labels2 = pd.read_table(f"{folds_dir}/fold2_labels.txt", header=None).values
    train_labels3 = pd.read_table(f"{folds_dir}/fold3_labels.txt", header=None).values
    train_labels = [
    ...
    val_images = pd.read_table(f"{folds_dir}/fold4_images.txt", header=None).values
    val_images = [os.path.join(image_dir, f[0]) for f in val_images]
    val_labels = pd.read_table(f"{folds_dir}/fold4_labels.txt", header=None).values
    val_labels = [os.path.join(label_dir, f[0]) for f in val_labels]

    test_images = pd.read_table(f"{folds_dir}/fold5_images.txt", header=None).values
    test_images = [os.path.join(image_dir, f[0]) for f in test_images]
    test_labels = pd.read_table(f"{folds_dir}/fold5_labels.txt", header=None).values
